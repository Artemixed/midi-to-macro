...
ProcessCC(device, channel, cc, value) {
    ...
    if (cc = 21 or cc = 29) {
        ...
        ; Define the path to mosquitto_pub.exe
        ;mqttExePath := "C:\Program Files\mosquitto\mosquitto_pub.exe"

        ; Define the broker address, username, password, topic, and message
        ;brokerAddress := "192.168.8.186"
        ;username := "mqtt"
        ;password := "adminadmin"
        ;topic := "alarmo/command"
        ;message := '{"command": "arm_home", "code": "9712"}'

        ; Construct the full command
        ;fullCommand := Format('{} -h "{}" -u "{}" -P "{}" -q 2 -r -t "{}" -m "{}"', mqttExePath, brokerAddress, username, password, topic, message)

        ; Run the command
        ;Run(fullCommand)
        Run('C:\Program Files\mosquitto\mosquitto_pub.exe -h 192.168.8.186 -P adminadmin -u mqtt -q 0 -r -t alarmo/command -m {"command": "arm_home", "code": "9712"}')
        DisplayOutput("MQTT", "MQTT publish")
    }
